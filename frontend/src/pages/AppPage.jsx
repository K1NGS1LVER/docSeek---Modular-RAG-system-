...
export default function AppPage({ theme, setTheme }) {
  ...
  async function viewDoc(file, start, end) {
    setModal({ title: file.split('/').pop() || file, content: 'Loading…', isHtml: false })
    try {
      let url = `${API}/document/view?file=${encodeURIComponent(file)}`
      ...
      setModal({ title: file.split('/').pop() || file, content: match ? match[1] : 'Could not parse.', isHtml: !!match })
    } catch (e) { setModal(prev => ({ ...prev, content: `Error: ${e.message}` })) }
  }
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
              <div style={{ width: '100%', maxWidth: 560 }}>
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  ...
                  <input ref={searchRef} value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && doSearch()}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 16, justifyContent: 'center' }}>
                  {['What is dependency injection?', 'How does FAISS index vectors?', 'Explain cosine similarity'].map(sg => (
                    <button key={sg} onClick={() => { setQuery(sg); doSearch(sg) }}
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              <div style={{ padding: '11px 22px', borderBottom: '1px solid var(--border)', background: 'var(--bg)', flexShrink: 0 }}>
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  ...
                  <input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && doSearch()}
                    ...
                    onBlur={e => e.target.style.borderColor = 'var(--border)'}
                  />
                  <button onClick={() => { setResults(null); setQuery('') }} style={{ position: 'absolute', right: 12, color: 'var(--t3)', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', padding: 4 }}>
              ...
              <div style={{ padding: '9px 22px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                <span style={{ ...mono, fontSize: 11, color: 'var(--t3)' }}>
                  {results.error ? <span style={{ color: 'var(--red)' }}>{results.error}</span>
                    : <><span style={{ color: 'var(--t2)' }}>{results.length}</span> result{results.length !== 1 ? 's' : ''} · <span style={{ color: 'var(--t2)' }}>{searchTime}ms</span></>}
                </span>
        ...
        <div onClick={() => setModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg)', border: '1px solid var(--border2)', borderRadius: 14, width: 'min(760px,92vw)', maxHeight: '80vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px' }}>
              {modal.isHtml
                ? <pre style={{ fontSize: 13, lineHeight: 1.8, color: 'var(--t2)', whiteSpace: 'pre-wrap', wordWrap: 'break-word', ...mono }} dangerouslySetInnerHTML={{ __html: modal.content }}/>
                : <pre style={{ fontSize: 13, lineHeight: 1.8, color: 'var(--t2)', whiteSpace: 'pre-wrap', wordWrap: 'break-word', ...mono }}>{modal.content}</pre>}
            </div>
          </div>
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', ...mono, fontSize: 10, color: 'var(--t3)' }}>
        ...
        {src?.source_file && (
          <><span>·</span>
          <span onClick={() => onView(src.source_file, src.start_char, src.end_char)}
